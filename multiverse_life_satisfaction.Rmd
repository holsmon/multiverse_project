...
mv$life_Composite <- (mv$SWLS_total + mv$DRS_Scaled) / 2

#Convert all mood_composite to z
...
life_analysis <- lm(life_z ~ branch(formula,
                   "GOAT" ~ GOAT_Z,
...
extract_variable_from_universe(M, 9, life_analysis) |> 
  broom::tidy()
...
#To filter, you can select which parts of the "universes" to plot. Here, I am excluding the covariates, which include age and race variables. 
  ...
  facet_wrap(~ analysis, scales = "free", nrow = 3) +
  ggtitle("Multiverse results for life satisfaction beta estimates") +
  theme_minimal() +
  theme(strip.text = element_blank()) +
